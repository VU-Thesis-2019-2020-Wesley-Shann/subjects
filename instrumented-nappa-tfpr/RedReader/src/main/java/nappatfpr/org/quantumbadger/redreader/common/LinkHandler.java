...
import nappatfpr.org.quantumbadger.redreader.fragments.UserProfileDialog;
import nappatfpr.org.quantumbadger.redreader.image.*;
	...
	public static boolean openWebBrowser(AppCompatActivity activity, Uri uri, final boolean fromExternalIntent) {
		...
		if(!fromExternalIntent) {
			...
			for (final ResolveInfo info : activity.getPackageManager().queryIntentActivities(baseIntent, 0)) {
				...
				if (packageName != null && !packageName.startsWith("nappatfpr.org.quantumbadger.redreader")) {
					final Intent intent = new Intent(Intent.ACTION_VIEW);
					intent.setData(uri);
