...
import nappatfpr.com.newsblur.database.ReadingAdapter;
import nappatfpr.com.newsblur.domain.Story;
import nappatfpr.com.newsblur.fragment.ReadingItemFragment;
import nappatfpr.com.newsblur.fragment.ReadingPagerFragment;
import nappatfpr.com.newsblur.service.NBSyncService;
...
import nappatfpr.com.newsblur.util.PrefConstants.ThemeValue;
import nappatfpr.com.newsblur.util.PrefsUtils;
import nappatfpr.com.newsblur.util.ReadingFontChangedListener;
...
import nappatfpr.com.newsblur.util.VolumeKeyNavigation;
import nappatfpr.com.newsblur.view.ReadingScrollView.ScrollChangeListener;
...
public abstract class Reading extends NbActivity implements OnPageChangeListener, OnSeekBarChangeListener, ScrollChangeListener, LoaderManager.LoaderCallbacks<Cursor>, ReadingFontChangedListener {
    ...
    @BindView(android.R.id.content) View contentView; // we use this a ton, so cache it
    @BindView(R.id.reading_overlay_left) Button overlayLeft;
    @BindView(R.id.reading_overlay_right) Button overlayRight;
    @BindView(R.id.reading_overlay_progress) ProgressBar overlayProgress;
    @BindView(R.id.reading_overlay_progress_right) ProgressBar overlayProgressRight;
    @BindView(R.id.reading_overlay_progress_left) ProgressBar overlayProgressLeft;
    @BindView(R.id.reading_overlay_text) Button overlayText;
    @BindView(R.id.reading_overlay_send) Button overlaySend;
    @BindView(R.id.reading_empty_view_text) View emptyViewText;
    @BindView(R.id.reading_sync_status) TextView overlayStatusText;
	...
	protected void onCreate(Bundle savedInstanceBundle) {
        ...
        try {
            ...
            // Serializable extras.
            nappatfpr.com.newsblur.util.Log.e(this, "failed to unfreeze required extras", re);
            finish();
            return;
        }

        if (fs == null) {
            nappatfpr.com.newsblur.util.Log.w(this.getClass().getName(), "reading view had no FeedSet");
            finish();
            return;
        }
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        synchronized (STORIES_MUTEX) {
            ...
            if (! FeedUtils.dbHelper.isFeedSetReady(fs)) {
                nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "stale load");
            ...
            nappatfpr.com.newsblur.util.Log.d(this.getClass().getName(), "loaded cursor with count: " + cursor.getCount());
