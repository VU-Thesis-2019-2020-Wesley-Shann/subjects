...
import nappatfpr.com.newsblur.activity.NbActivity;
import nappatfpr.com.newsblur.database.BlurDatabaseHelper;
import static nappatfpr.com.newsblur.database.BlurDatabaseHelper.closeQuietly;
import nappatfpr.com.newsblur.database.DatabaseConstants;
import nappatfpr.com.newsblur.domain.Feed;
import nappatfpr.com.newsblur.domain.Folder;
import nappatfpr.com.newsblur.domain.SocialFeed;
import nappatfpr.com.newsblur.domain.StarredCount;
import nappatfpr.com.newsblur.domain.Story;
import nappatfpr.com.newsblur.network.APIConstants;
import nappatfpr.com.newsblur.network.APIManager;
import nappatfpr.com.newsblur.network.domain.FeedFolderResponse;
import nappatfpr.com.newsblur.network.domain.NewsBlurResponse;
import nappatfpr.com.newsblur.network.domain.StoriesResponse;
import nappatfpr.com.newsblur.network.domain.UnreadCountResponse;
import nappatfpr.com.newsblur.util.AppConstants;
import nappatfpr.com.newsblur.util.DefaultFeedView;
import nappatfpr.com.newsblur.util.FeedSet;
import nappatfpr.com.newsblur.util.FileCache;
import nappatfpr.com.newsblur.util.Log;
import nappatfpr.com.newsblur.util.NetworkUtils;
import nappatfpr.com.newsblur.util.NotificationUtils;
import nappatfpr.com.newsblur.util.PrefsUtils;
import nappatfpr.com.newsblur.util.ReadingAction;
import nappatfpr.com.newsblur.util.ReadFilter;
import nappatfpr.com.newsblur.util.StateFilter;
import nappatfpr.com.newsblur.util.StoryOrder;
...
public class NBSyncService extends JobService {
	...
	public void onCreate() {
		super.onCreate();
        nappatfpr.com.newsblur.util.Log.d(this, "onCreate");
        HaltNow = false;
    ...
    private void finishConstruction() {
        if ((apiManager == null) || (dbHelper == null)) {
            ...
            nappatfpr.com.newsblur.util.Log.offerContext(this);
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, final int startId) {
        nappatfpr.com.newsblur.util.Log.d(this, "onStartCommand");
        ...
        if ((NbActivity.getActiveActivityCount() > 0) || PrefsUtils.isBackgroundNeeded(this)) {
            ...
            nappatfpr.com.newsblur.util.Log.i(this, "Skipping sync: app not active and background sync not enabled.");
            synchronized (COMPLETION_CALLBACKS_MUTEX) {outstandingStartIds.add(startId);}
            checkCompletion();
        } 
    ...
    public boolean onStartJob(final JobParameters params) {
        nappatfpr.com.newsblur.util.Log.d(this, "onStartJob");
        // only perform a sync if the app is actually running or background syncs are enabled
        if ((NbActivity.getActiveActivityCount() > 0) || PrefsUtils.isBackgroundNeeded(this)) {
            HaltNow = false;
            // Services actually get invoked on the main system thread, and are not
            ...
            nappatfpr.com.newsblur.util.Log.d(this, "Skipping sync: app not active and background sync not enabled.");
            synchronized (COMPLETION_CALLBACKS_MUTEX) {outstandingStartParams.add(params);}
            checkCompletion();
        } 
    ...
    public boolean onStopJob(JobParameters params) {
        nappatfpr.com.newsblur.util.Log.d(this, "onStopJob");
        HaltNow = true;
        ...
        try {
            ...
            if (OfflineNow) {
                if (NetworkUtils.isOnline(this)) {
                    ...
                    nappatfpr.com.newsblur.util.Log.d(this, "Abandoning sync: network still offline");
                    return;
                }
            }
            ...
            nappatfpr.com.newsblur.util.Log.e(this.getClass().getName(), "Sync error.", e);
        } 
    }
    ...
    private void housekeeping() {
        try {
            ...
            if (upgraded || autoVac) {
                HousekeepingRunning = true;
                NbActivity.updateAllActivities(NbActivity.UPDATE_STATUS);
                nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "rebuilding DB . . .");
                dbHelper.vacuum();
                nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), ". . . . done rebuilding DB");
        ...
        try {
            ...
            actionsloop : while (c.moveToNext()) {
                ...
                try {
                    ...
                    nappatfpr.com.newsblur.util.Log.e(this.getClass().getName(), "error unfreezing ReadingAction", e);
                ...
                nappatfpr.com.newsblur.util.Log.d(this, "attempting action: " + ra.toContentValues().toString());
                NewsBlurResponse response = ra.doRemote(apiManager, dbHelper);

                if (response == null) {
                    nappatfpr.com.newsblur.util.Log.e(this.getClass().getName(), "Discarding reading action with client-side error.");
                    dbHelper.clearAction(id);
                } else if (response.isProtocolError) {
                    // the network failed or we got a non-200, so be sure we retry
                    nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "Holding reading action with server-side or network error.");
                    ...
                    nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "Discarding reading action with fatal message.");
                    dbHelper.clearAction(id);
    ...
    private void syncMetadata() {
        ...
        if (untriedActions > 0) {
            nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), untriedActions + " outstanding actions, yielding metadata sync");
            return;
        }
        ...
        nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "ready to sync feed list");
        ...
        try {
            ...
            if (! feedResponse.isAuthenticated) {
                ...
                authFails += 1;
                nappatfpr.com.newsblur.util.Log.w(this.getClass().getName(), "Server ignored or rejected auth cookie.");
                if (authFails >= AppConstants.MAX_API_TRIES) {
                    nappatfpr.com.newsblur.util.Log.w(this.getClass().getName(), "too many auth fails, resetting cookie");
            ...
            nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "got feed list: " + getSpeedInfo());
    ...
    private void checkRecounts() {
        ...
        try {
            ...
            nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "recounting dirty feed sets: " + dirtySets.size());
            ...
            if (!NetworkUtils.isOnline(this)) {
                ...
                if ((apiResponse == null) || (apiResponse.isError())) {
                    nappatfpr.com.newsblur.util.Log.w(this.getClass().getName(), "Bad response to feed_unread_count");
                    return;
                }
    ...
    private void syncPendingFeedStories() {
        ...
        try {
            ...
            synchronized (MUTEX_ResetFeed) {
                if (ResetFeed != null) {
                    nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "Resetting state for feed set: " + ResetFeed);
            ...
            if (fs == null) {
                nappatfpr.com.newsblur.util.Log.d(this.getClass().getName(), "No feed set to sync");
                return;
            }
            ...
            if (ExhaustedFeeds.contains(fs)) {
                nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "No more stories for feed set: " + fs);
                finished = true;
                return;
            }
    ...
    private boolean isStoryResponseGood(StoriesResponse response) {
        if (response == null) {
            nappatfpr.com.newsblur.util.Log.e(this.getClass().getName(), "Null response received while loading stories.");
            return false;
        }
        if (response.stories == null) {
            nappatfpr.com.newsblur.util.Log.e(this.getClass().getName(), "Null stories member received while loading stories.");
            return false;
        }
        return true;
    }
    ...
    private void insertStories(StoriesResponse apiResponse, FeedSet fs) {
        ...
        nappatfpr.com.newsblur.util.Log.d(NBSyncService.class.getName(), "got stories from main fetch loop: " + apiResponse.stories.length);
        dbHelper.insertStories(apiResponse, true);
    }

    void insertStories(StoriesResponse apiResponse) {
        nappatfpr.com.newsblur.util.Log.d(NBSyncService.class.getName(), "got stories from sub sync: " + apiResponse.stories.length);
    ...
    static boolean stopSync(Context context) {
        if (HaltNow) {
            nappatfpr.com.newsblur.util.Log.i(NBSyncService.class.getName(), "stopping sync, soft interrupt set.");
            return true;
        }
    ...
    private void noteHardAPIFailure() {
        nappatfpr.com.newsblur.util.Log.w(this.getClass().getName(), "hard API failure");
    ...
    private boolean backoffBackgroundCalls() {
        ...
        nappatfpr.com.newsblur.util.Log.i(this.getClass().getName(), "abandoning background sync due to recent API failures.");
        return true;
    }
    ...
    public static void prepareReadingSession(BlurDatabaseHelper dbHelper, FeedSet fs) {
        synchronized (PENDING_FEED_MUTEX) {
            if (! fs.equals(dbHelper.getSessionFeedSet())) {
                nappatfpr.com.newsblur.util.Log.d(NBSyncService.class.getName(), "preparing new reading session");
    ...
    public static void resetReadingSession(BlurDatabaseHelper dbHelper) {
        nappatfpr.com.newsblur.util.Log.d(NBSyncService.class.getName(), "requesting reading session reset");
        synchronized (PENDING_FEED_MUTEX) {
            PendingFeed = null;
            dbHelper.setSessionFeedSet(null);
        }
    ...
    public static void resetFetchState(FeedSet fs) {
        synchronized (MUTEX_ResetFeed) {
            nappatfpr.com.newsblur.util.Log.d(NBSyncService.class.getName(), "requesting feed fetch state reset");
            ResetFeed = fs;
        }
    }
    ...
    public static void softInterrupt() {
        nappatfpr.com.newsblur.util.Log.i(NBSyncService.class.getName(), "soft stop");
        HaltNow = true;
    }
    ...
    public void onDestroy() {
        try {
            nappatfpr.com.newsblur.util.Log.d(this, "onDestroy");
            synchronized (COMPLETION_CALLBACKS_MUTEX) {
                if ((outstandingStartIds.size() > 0) || (outstandingStartParams.size() > 0)) {
                    nappatfpr.com.newsblur.util.Log.w(this, "Service scheduler destroyed before all jobs marked done?");
                }
            }
            ...
            nappatfpr.com.newsblur.util.Log.d(this, "onDestroy done");
        } catch (Exception ex) {
            nappatfpr.com.newsblur.util.Log.e(this, "unclean shutdown", ex);
        }
        super.onDestroy();
    }
