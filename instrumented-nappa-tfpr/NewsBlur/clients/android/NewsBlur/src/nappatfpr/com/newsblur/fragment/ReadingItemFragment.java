...
import nappatfpr.com.newsblur.activity.Reading;
...
import nappatfpr.com.newsblur.service.OriginalTextService;
...
import nappatfpr.com.newsblur.util.Font;
import nappatfpr.com.newsblur.util.PrefConstants.ThemeValue;
import nappatfpr.com.newsblur.util.PrefsUtils;
import nappatfpr.com.newsblur.util.StoryUtils;
import nappatfpr.com.newsblur.util.UIUtils;
import nappatfpr.com.newsblur.util.ViewUtils;
import nappatfpr.com.newsblur.view.FlowLayout;
import nappatfpr.com.newsblur.view.NewsblurWebview;
import nappatfpr.com.newsblur.view.ReadingScrollView;
...
public class ReadingItemFragment extends NbFragment implements PopupMenu.OnMenuItemClickListener {
    ...
    public boolean onMenuItemClick(MenuItem item) {
		if (item.getItemId() == R.id.menu_reading_original) {
            ...
            try {
                ...
                nappatfpr.com.newsblur.util.Log.e(this, "device cannot open URLs");
            }
			return true;
    ...
    public void offerStoryUpdate(Story story) {
        ...
        if (! TextUtils.equals(story.storyHash, this.story.storyHash)) {
            nappatfpr.com.newsblur.util.Log.d(this, "prevented story list index offset shift");
            return;
        }
        this.story = story;
        //if (AppConstants.VERBOSE_LOG) nappatfpr.com.newsblur.util.Log.d(this, "got fresh story");
    }
                ...
                protected void onPostExecute(String result) {
                    if (result != null) {
                        if (OriginalTextService.NULL_STORY_TEXT.equals(result)) {
                            ...
                            nappatfpr.com.newsblur.util.Log.d(this, "orig text not avail for story: " + story.storyHash);
                        ...
                        nappatfpr.com.newsblur.util.Log.d(this, "orig text not yet cached for story: " + story.storyHash);
        ...
        new AsyncTask<Void, Void, String>() {
            ...
            protected void onPostExecute(String result) {
                if (result != null) {
                    ...
                    nappatfpr.com.newsblur.util.Log.w(this, "couldn't find story content for existing story.");
