...
import nappatfpr.com.newsblur.activity.*;
...
public class UIUtils {
     ...
     *
     * @param styleId the style that defines the attr, such as nappatfpr.com.newsblur.R.attr.defaultText
    ...
    public static int getThemedColor(Context context, int styleId, int rId) {
        ...
        if (val.peekValue(0).type != TypedValue.TYPE_REFERENCE) {
            nappatfpr.com.newsblur.util.Log.w(UIUtils.class.getName(), "styleId didn't resolve to a style");
            val.recycle();
        ...
        if (effectiveStyleId == -1) {
            nappatfpr.com.newsblur.util.Log.w(UIUtils.class.getName(), "styleId didn't resolve to a known style");
            return Color.MAGENTA;
        }
        int[] attrs2 = {rId};
        ...
        if ( (val2.peekValue(0).type < TypedValue.TYPE_FIRST_COLOR_INT) || (val2.peekValue(0).type > TypedValue.TYPE_LAST_COLOR_INT)) {
            nappatfpr.com.newsblur.util.Log.w(UIUtils.class.getName(), "rId didn't resolve to a color within given style");
            val2.recycle();
            return Color.MAGENTA;
        }
    ...
    public static int getThemedResource(Context context, int styleId, int rId) {
        ...
        try {
            ...
            nappatfpr.com.newsblur.util.Log.w(UIUtils.class.getName(), "rId didn't resolve to a drawable within given style");
        }
        val2.recycle();
        return result;
    }
    ...
    public static int decodeColourValue(String val, int defaultVal) {
        ...
        try {
            ...
            nappatfpr.com.newsblur.util.Log.e(UIUtils.class.getName(), "feed supplied bad color info: " + nfe.getMessage());
        }
        return result;
    }
