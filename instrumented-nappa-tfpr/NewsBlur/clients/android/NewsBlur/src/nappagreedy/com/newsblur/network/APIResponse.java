...
public class APIResponse {
    ...
    public APIResponse(Context context, OkHttpClient httpClient, Request request, int expectedReturnCode) {
        ...
        try {
            ...
            if (responseCode != expectedReturnCode) {
                nappagreedy.com.newsblur.util.Log.e(this.getClass().getName(), "API returned error code " + response.code() + " calling " + request.url().toString() + " - expected " + expectedReturnCode);
                this.isError = true;
                return;
            }
            ...
            try {
                ...
                nappagreedy.com.newsblur.util.Log.e(this.getClass().getName(), e.getClass().getName() + " (" + e.getMessage() + ") reading " + request.url().toString(), e);
                this.isError = true;
                return;
            }
            ...
            nappagreedy.com.newsblur.util.Log.d(this.getClass().getName(), String.format("called %s in %dms and %dms to read %dB", request.url().toString(), connectTime, readTime, responseBody.length()));

        } catch (IOException ioe) {
            nappagreedy.com.newsblur.util.Log.e(this.getClass().getName(), "Error (" + ioe.getMessage() + ") calling " + request.url().toString(), ioe);
            this.isError = true;
            return;
        }
    }
    ...
    public APIResponse(Context context) {
        nappagreedy.com.newsblur.util.Log.w(this.getClass().getName(), "failing an offline API response");
        this.isError = true;
    }
