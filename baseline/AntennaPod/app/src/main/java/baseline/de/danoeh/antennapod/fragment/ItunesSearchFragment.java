...
import baseline.de.danoeh.antennapod.adapter.itunes.ItunesAdapter;
...
import static baseline.de.danoeh.antennapod.adapter.itunes.ItunesAdapter.Podcast;
...
public class ItunesSearchFragment extends Fragment {
        ...
        gridView.setOnItemClickListener((parent, view1, position, id) -> {
            ...
            if (!podcast.feedUrl.contains("itunes.apple.com")) {
                ...
                disposable = Single.create((SingleOnSubscribe<String>) emitter -> {
                            ...
                            try {
                                Call temp = client.newCall(httpReq.build());
    ...
    private String getTopListFeed(OkHttpClient client, String language) throws IOException {
        ...
        Call temp = client.newCall(httpReq.build());
        long sentRequestAtMillis = System.currentTimeMillis();
        try (Response response = temp.execute()) {
            long receivedResponseAtMillis = System.currentTimeMillis();
            MetricNetworkRequestExecutionTime.log(response, sentRequestAtMillis, receivedResponseAtMillis, true);
            if (response.isSuccessful()) {
